...
/* ---------- Onboarding wizard (topics → cities → keywords) ---------- */
function initOnboarding(){
  ...
  const skipBtns = wizard.querySelectorAll('[data-wizard-skip]');
  const backBtns = wizard.querySelectorAll('[data-wizard-back]');
  let current = 0;
  ...
  nextBtns.forEach(btn => btn.addEventListener('click', () => {
    if(current === panels.length - 1){
      ...
      setTimeout(() => window.location.href = wizard.dataset.finishUrl || 'dashboard.html', 500);
    } else {
      goTo(current + 1);
    }
  }));
  skipBtns.forEach(btn => btn.addEventListener('click', () => {
    if(current === panels.length - 1){
      window.location.href = wizard.dataset.finishUrl || 'dashboard.html';
    } else {
      goTo(current + 1);
    }
  }));
  backBtns.forEach(btn => btn.addEventListener('click', () => goTo(current - 1)));

  render();

  // Selectable pills / rows (topics, cities, suggested keywords)
  wizard.querySelectorAll('[data-selectable]').forEach(group => {
    const countLabel = group.parentElement.querySelector('[data-selected-count]');
    function updateCount(){
      const n = group.querySelectorAll('.selected').length;
      if(countLabel) countLabel.textContent = n + (n > 1 ? ' sélections' : ' sélection');
    }
    group.querySelectorAll('.option-pill, .option-row').forEach(opt => {
  ...
  // Search filter for the city / keyword lists
  wizard.querySelectorAll('[data-option-search]').forEach(input => {
    const list = wizard.querySelector(input.dataset.optionSearch);
    if(!list) return;
    input.addEventListener('input', () => {
  ...
  // Custom keyword chips (step 3 "add your own")
  const kwInput = wizard.querySelector('[data-custom-keyword-input]');
  const kwAddBtn = wizard.querySelector('[data-custom-keyword-add]');
  const kwChipRow = wizard.querySelector('[data-custom-keyword-chips]');
  if(kwInput && kwAddBtn && kwChipRow){
    ...
    kwAddBtn.addEventListener('click', addCustomKeyword);
